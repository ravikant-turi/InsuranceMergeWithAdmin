...
// add/register pharmacy
	...
	public String addPharmacy(Pharmacy pharmacy) {
		...
		// test.....
		System.out.println("Saving pharmacy: " + pharmacy.getPharmacyId() + " | Owner: " + pharmacy.getFirstName() + " "
				+ pharmacy.getLastName() + " | Pharmacy: " + pharmacy.getPharmacyName());
	...
	public String updatePassword(String email, String pwd) {
		...
		if (pharmacy != null) {
			...
			if (otp != null) {
				...
				Query cleanUp = session
						.createQuery("delete from PharmacyOtp where pharmacyId = :pharmacyId and status != :status");
	...
	public String resendOtp(String email) {
		...
		Transaction tx = session.beginTransaction();
		Query otpUpdate = session.createQuery(
				"update PharmacyOtp set status = :expired where pharmacyId = :pharmacyId and status = :pending");
